# Adapted verbatim from https://github.com/ZNLP/KTAE
# (commit on main as of 2026-05-06).
#
# KTAE (Key-Token Advantage Estimation) computes a per-token weight from
# group-internal correctness statistics: Fisher exact test, Information
# Gain, and Cohen's h with optional BM25-style TF correction.
#
# This file is a baseline integration for the SentencePO paper. We keep
# the source structure unchanged for fidelity to the released code; only
# the import block is reduced to what we actually need.
...
def fisher_exact_test(input_tensor):
    ...
    k_grid = torch.tile(k_values, (input_tensor.size(0), 1))
    ...
    p_values = torch.sum(torch.where((p_k <= p_obs.unsqueeze(1)) & mask, p_k, 0.0), dim=1)
...
def calculate_ig(table_tensor):
    ...
    h_y_x = ((t_a + t_b) / t_n) * conditional_entropy(t_a, t_b) + ((t_c + t_d) / t_n) * conditional_entropy(t_c, t_d)
    ig = h_y - h_y_x
    ig[t_n == 0] = 0
    return ig
...
def compute_tf(correct_tensor, correct_mask, wrong_tensor, wrong_mask, appear_table_tensor):
    ...
    tf_tensor = torch.stack(
        ...
        dim=1,
    ).to(dtype=torch.float32, device=appear_table_tensor.device)
...
class ComputeKeyTokens:
    def __init__(self, alpha, beta_ig, gamma_tf, top, bottom, responses_ids, mask, rewards, max_token_num,
        ...
        # KTAE upstream hardcodes 151643 (Qwen pad). Parameterised here so
        # Llama / other tokenisers can override via algorithm.ktae.pad_token_id.
    ...
    def get_distribution(self, tensor, mask):
        ...
        row_indices = torch.arange(m)[:, None].expand(m, n)
    ...
    def compute_key_tokens(self, correct_tensor, wrong_tensor, correct_mask, wrong_mask):
        ...
        ig_val = (
            calculate_ig(appear_table_tensor)
            if self.beta_ig
            else torch.zeros(self.max_token_num + 1, dtype=torch.float32, device=appear_table_tensor.device)
        )
        if self.alpha:
            fisher_pvalue = fisher_exact_test(appear_table_tensor)
            format_fisher_pvalue = torch.where(
                fisher_pvalue != 1.0,
                torch.exp(-2 * fisher_pvalue),
                torch.tensor(0.0, dtype=torch.float32, device=appear_table_tensor.device),
            )
        else:
            format_fisher_pvalue = torch.zeros(
        ...
        if self.gamma_tf:
            ...
            x = compute_Cohen(
                appear_table_tensor[:, 0] / (appear_table_tensor[:, 0] + appear_table_tensor[:, 2] + 1e-10),
                appear_table_tensor[:, 1] / (appear_table_tensor[:, 1] + appear_table_tensor[:, 3] + 1e-10),
            )
        ...
        # Zero-out the pad token weight (KTAE upstream hardcodes Qwen pad id 151643).
        if 0 <= self.pad_token_id <= self.max_token_num:
            key_token_result[self.pad_token_id] = 0.0
        return key_token_result
    ...
    def get_key_tokens(self):
        ...
        key_token_result = self.compute_key_tokens(
            correct_outputs, wrong_outputs, correct_outputs_mask, wrong_outputs_mask
        )
        return key_token_result
